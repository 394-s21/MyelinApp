...
describe('<PrebuiltTaskScreen />', () => {
  it('Renders correctly', async () => {
    const { screenTest } = render(
      <PrebuiltTaskScreen route={{ params: { thisUser: dummyUser } }} />
    )
...
it('Moves to view message screen', async () => {
  ...
  const { getAllByText, getByTestId } = render(
    <PrebuiltTaskScreen
      ...
      route={{ params: { thisUser: dummyUser, task: dummyTask } }}
    />
  )

  fireEvent.press(getByTestId('PrebuiltTask.CreateTask'))
  expect(navigation.navigate).toBeCalledWith('CreateTaskScreen', {
    task: task,
    thisUser: dummyUser,
  })
})
